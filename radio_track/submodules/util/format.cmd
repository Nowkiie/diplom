...
@CALL :FormatFilesInDirectory %CD%\include
@CALL :FormatFilesInDirectory %CD%\source
@CALL :FormatFilesInDirectory %CD%\tests
@GOTO :EOF
...
@ECHO.
@PUSHD %DIRECTORY%
@FOR /F "tokens=*" %%F IN ('DIR /B/S/A-D *.cpp *.hpp') DO @ECHO Processing file "%%F" & @%CLANG_FORMAT% -i -style=file %%F
@POPD
