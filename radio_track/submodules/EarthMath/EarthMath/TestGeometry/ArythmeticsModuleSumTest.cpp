...
BOOST_AUTO_TEST_SUITE(ArithmeticSum)
using namespace earth_math;
using degree360 = degree_module<360>;

BOOST_AUTO_TEST_CASE(test_simple_sum)
{
    const auto a = degree360::from_degree(30.);
    const auto b = degree360::from_degree(180.);
    const auto c = a + b;
    BOOST_CHECK_EQUAL(c.to_string(show_seconds), "210\xC2\xB0 00' 00''");
}
BOOST_AUTO_TEST_CASE(test_overload_sum)
{
    const auto a = degree360::from_degree(30.);
    const auto b = degree360::from_degree(340.);
    const auto c = a + b;
    BOOST_CHECK_EQUAL(c.to_string(show_seconds), "10\xC2\xB0 00' 00''");
}
BOOST_AUTO_TEST_CASE(test_many_overload_sum)
{
    const auto a = degree360::from_degree(30.);
    const auto b = degree360::from_degree(340.);
    const auto c = a + b + b + b;
    BOOST_CHECK_EQUAL(c.to_string(show_seconds), "330\xC2\xB0 00' 00''");
}
