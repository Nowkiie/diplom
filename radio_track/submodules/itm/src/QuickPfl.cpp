...
/*=============================================================================
 ...
 |
 *===========================================================================*/
void QuickPfl(double pfl1[], double gamma_e, double h__meter[2], double theta_hzn[2], 
    double d_hzn__meter[2], double h_e__meter[2], double *delta_h__meter, double *d__meter)
{
    std::vector<double> pfl(pfl1[0] + 2);
    for (int i = 0; i < pfl1[0] + 2; i++)
        pfl[i] = pfl1[i];
    ...
    *delta_h__meter = ComputeDeltaH(pfl, d_start__meter, d_end__meter);
    ...
    if (d_hzn__meter[0] + d_hzn__meter[1] > 1.5 * *d__meter)
    {
        ...
        h_e__meter[1] = h__meter[1] + fdim(pfl[np + 1], fit_rx); //BUG!!! pfl[np + 2]
