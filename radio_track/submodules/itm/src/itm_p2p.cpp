    ...
    double *A__db, long *warnings, IntermediateValues *interValues)
{
    ...
    *warnings = NO_WARNINGS;    // Initialize to no warnings
    ...
    if (np < 5) {  // BUG!!! without this all crashes cause of array bounds errors
        d__meter = pfl[0] * pfl[1];
        interValues->A_fs__db = FreeSpaceLoss(d__meter, f__mhz);
        *A__db = interValues->A_fs__db + 20;
        interValues->A_ref__db = 20;
        return SUCCESS;
    }
    ...
    for (int i = p10; i <= std::min(np-1, np - p10); i++) //BUG!!! if p10 = 0 -> array bound error
