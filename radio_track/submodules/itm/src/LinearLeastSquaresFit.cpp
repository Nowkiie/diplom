...
void LinearLeastSquaresFit(std::vector<double> &pfl, double d_start, double d_end, double *fit_y1, double *fit_y2)
{
    int np = (int)pfl[0];
    ...
    if (i_end <= i_start)
    {
        ...
        i_end = std::min(np - 1, np - (int)fdim(np, i_end + 1.0));
    }
    ...
    double scaled_sum_y = 0.5 * (pfl[i_start + 2] - pfl[i_end + 2]) * mid_shifted_index; //BUG!!! 
