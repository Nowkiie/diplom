...
// ITM DLL Functions
typedef int(__stdcall* itm_area_tls_ex_func)(
    double h_tx__meter, double h_rx__meter, int tx_site_criteria,
    int rx_site_criteria, double d__km, double delta_h__meter, int climate,
    double N_0, double f__mhz, int pol, double epsilon, double sigma, int mdvar,
    double time, double location, double situation, double* A__db,
    long* warnings, struct IntermediateValues* interValues);

typedef int(__stdcall* itm_area_cr_ex_func)(
    double h_tx__meter, double h_rx__meter, int tx_site_criteria,
    int rx_site_criteria, double d__km, double delta_h__meter, int climate,
    double N_0, double f__mhz, int pol, double epsilon, double sigma, int mdvar,
    double confidence, double reliability, double* A__db, long* warnings,
    struct IntermediateValues* interValues);
...
/*=============================================================================
 ...
 |
 *===========================================================================*/
int CallAreaMode(DrvrParams* params, AreaParams* area_params,
                 IntermediateValues* inter_vals, vector<double>* A__db,
                 vector<double>* A_fs__db, long* warnings)
{
    ...
    *warnings = 0;
    ...
    if (area_params->mode == VARY_TLS)
    {
        for (int i = 0; i < area_params->d__km.size(); i++)
        {
            ...
            {
                rtn = itm_area_tls_ex(
                    area_params->h_tx__meter, area_params->h_rx__meter,
                    area_params->tx_site_criteria,
                    area_params->rx_site_criteria, area_params->d__km[i],
                    area_params->delta_h__meter, area_params->climate,
                    area_params->N_0, area_params->f__mhz, area_params->pol,
                    area_params->epsilon, area_params->sigma,
                    area_params->mdvar, area_params->time,
                    area_params->location, area_params->situation[j], &ADB,
                    &WARN, inter_vals);
        ...
        for (int i = 0; i < area_params->d__km.size(); i++)
        {
            for (int j = 0; j < area_params->confidence.size(); j++)
            {
                rtn = itm_area_cr_ex(
                    ...
                    area_params->N_0, area_params->f__mhz, area_params->pol,
                    area_params->epsilon, area_params->sigma,
                    area_params->mdvar, area_params->confidence[j],
                    area_params->reliability, &ADB, &WARN, inter_vals);
...
{
    itm_area_tls_ex =
        (itm_area_tls_ex_func)GetProcAddress((HMODULE)hLib, "ITM_AREA_TLS_Ex");
    if (itm_area_tls_ex == nullptr)
        return DRVRERR__GET_AREA_TLS_FUNC_LOADING;

    itm_area_cr_ex =
        (itm_area_cr_ex_func)GetProcAddress((HMODULE)hLib, "ITM_AREA_CR_Ex");
...
int ParseAreaInputFile(const char* in_file, AreaParams* area_params)
{
    ...
    while (getline(file, line))
    {
        ...
        {
            if (ParseInteger(value.c_str(), &area_params->tx_site_criteria)
                == ERROR)
            {
        ...
        {
            if (ParseInteger(value.c_str(), &area_params->rx_site_criteria)
                == ERROR)
            {
        ...
        else if (key.compare(TAG__DKM) == 0)
        {
            ...
            {
                return ParsingErrorHelper(DRVRERR__PARSE_INCOMPLETE_DISTANCES,
                                          TAG__DKM);
            }
            else
            {
                ...
                do
                {
                    d__km = area_params->d_format[0]
                            + i1 * area_params->d_format[2];
        ...
        {
            if (ParseDouble(value.c_str(), &area_params->delta_h__meter)
                == ERROR)
            {
        ...
        else if (key.compare(TAG__LOCATION) == 0)
        {
            ...
            {
                return ParsingErrorHelper(DRVRERR__PARSE_LOCATION,
                                          TAG__LOCATION);
            }
        }
        else if (key.compare(TAG__SITUATION) == 0)
        {
            ...
            do
            {
                ...
                {
                    return ParsingErrorHelper(DRVRERR__PARSE_SITUATION,
                                              TAG__SITUATION);
                }
        ...
        else if (key.compare(TAG__CONFIDENCE) == 0)
        {
            ...
            do
            {
                ...
                {
                    return ParsingErrorHelper(DRVRERR__PARSE_CONFIDENCE,
                                              TAG__CONFIDENCE);
                }
        ...
        else if (key.compare(TAG__RELIABILITY) == 0)
        {
            ...
            {
                return ParsingErrorHelper(DRVRERR__PARSE_RELIABILITY,
                                          TAG__RELIABILITY);
            }
        }
        else
        {
            printf(
                "Unknown area mode input parameter '%s'.  Skipping line...\n",
                key.c_str());
        }
    }
...
int ValidateAreaInputs(AreaParams* area_params)
{
    ...
    if (area_params->tx_site_criteria == NOT_SET)
        return Validate_RequiredErrMsgHelper(TAG__TX_SITE,
                                             DRVRERR__VALIDATION_HTX_SITE);

    if (area_params->rx_site_criteria == NOT_SET)
        return Validate_RequiredErrMsgHelper(TAG__RX_SITE,
                                             DRVRERR__VALIDATION_RTX_SITE);
    ...
    if (area_params->delta_h__meter == NOT_SET)
        return Validate_RequiredErrMsgHelper(TAG__DELTAH,
                                             DRVRERR__VALIDATION_DELTAH);

    if (area_params->climate == NOT_SET)
        return Validate_RequiredErrMsgHelper(TAG__CLIMATE,
                                             DRVRERR__VALIDATION_CLIMATE);
    ...
    if (area_params->f__mhz == NOT_SET)
        return Validate_RequiredErrMsgHelper(TAG__FREQ,
    ...
    if (area_params->epsilon == NOT_SET)
        return Validate_RequiredErrMsgHelper(TAG__EPSILON,
                                             DRVRERR__VALIDATION_EPSILON);

    if (area_params->sigma == NOT_SET)
        return Validate_RequiredErrMsgHelper(TAG__SIGMA,
                                             DRVRERR__VALIDATION_SIGMA);

    if (area_params->mdvar == NOT_SET)
        return Validate_RequiredErrMsgHelper(TAG__MDVAR,
                                             DRVRERR__VALIDATION_MDVAR);

    // infer TLS vs CR calling
    bool isTLS =
        (area_params->time != NOT_SET && area_params->location != NOT_SET
         && area_params->situation.size() > 0);
    bool isCR = (area_params->confidence.size() > 0
                 && area_params->reliability != NOT_SET);

    if (isTLS && isCR)
        return GeneralErrorMsgHelper(
            "Provided both time/location/situation parameters and "
            "confidence/reliability",
            DRVRERR__VALIDATION_TLS_AND_CR);

    if (!isTLS && !isCR)
        return GeneralErrorMsgHelper(
            "Must provide time/location/situation parameters or "
            "confidence/reliability",
            DRVRERR__VALIDATION_TLS_OR_CR);
...
{
    fprintf_s(fp, "%-24s %-12g %s\n", TAG__HTX, area_params->h_tx__meter,
              UNITS__METER);
    fprintf_s(fp, "%-24s %-12g %s\n", TAG__HRX, area_params->h_rx__meter,
              UNITS__METER);
    ...
    {
        fprintf_s(fp, "%-24s %-12g %s\n", TAG__DKM, area_params->d__km.front(),
                  UNITS__KM);
    }
    else
    {
        fprintf_s(fp, "%-24s %g,%g,%g,%g,%g\n", TAG__DKM,
                  area_params->d_format[0], area_params->d_format[1],
                  area_params->d_format[2], area_params->d_format[3],
                  area_params->d_format[4]);
    }
    fprintf_s(fp, "%-24s %-12g %s\n", TAG__DELTAH, area_params->delta_h__meter,
              UNITS__METER);
    fprintf_s(fp, "%-24s %-12i ", TAG__CLIMATE, area_params->climate);
    PrintClimateLabel(fp, area_params->climate);
    fprintf_s(fp, "%-24s %-12.2f %s\n", TAG__N0, area_params->N_0,
              UNITS__NUNIT);
    fprintf_s(fp, "%-24s %-12.2f %s\n", TAG__FREQ, area_params->f__mhz,
              UNITS__MHZ);
