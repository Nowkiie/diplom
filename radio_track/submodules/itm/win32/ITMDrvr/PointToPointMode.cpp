...
// ITM DLL Functions
typedef int(__stdcall* itm_p2p_tls_ex_func)(
    double h_tx__meter, double h_rx__meter, double pfl[], int climate,
...
typedef int(__stdcall* itm_p2p_cr_ex_func)(
    double h_tx__meter, double h_rx__meter, double pfl[], int climate,
    double N_0, double f__mhz, int pol, double epsilon, double sigma, int mdvar,
...
/*=============================================================================
 ...
 |
 *===========================================================================*/
int CallP2PMode(DrvrParams* params, P2PParams* p2p_params,
                ...
                long* warnings)
{
    ...
    {
        rtn = itm_p2p_tls_ex(
            p2p_params->h_tx__meter, p2p_params->h_rx__meter, &pfl[0],
            p2p_params->climate, p2p_params->N_0, p2p_params->f__mhz,
            p2p_params->pol, p2p_params->epsilon, p2p_params->sigma,
            p2p_params->mdvar, p2p_params->time, p2p_params->location,
            p2p_params->situation, &ADB, warnings, inter_vals);
        ...
        for (int i = 0; i < p2p_params->reliability.size(); i++)
        {
            for (int j = 0; j < p2p_params->confidence.size(); j++)
            {
                rtn = itm_p2p_cr_ex(
                    ...
                    p2p_params->climate, p2p_params->N_0, p2p_params->f__mhz,
                    p2p_params->pol, p2p_params->epsilon, p2p_params->sigma,
                    p2p_params->mdvar, p2p_params->confidence[j],
                    p2p_params->reliability[i], &ADB, &WARN, inter_vals);
...
{
    itm_p2p_tls_ex =
        (itm_p2p_tls_ex_func)GetProcAddress((HMODULE)hLib, "ITM_P2P_TLS_Ex");
    if (itm_p2p_tls_ex == nullptr)
        return DRVRERR__GET_P2P_TLS_FUNC_LOADING;

    itm_p2p_cr_ex =
        (itm_p2p_cr_ex_func)GetProcAddress((HMODULE)hLib, "ITM_P2P_CR_Ex");
...
int ValidateP2PInputs(P2PParams* p2p_params)
{
    ...
    bool isTLS = (p2p_params->time != NOT_SET && p2p_params->location != NOT_SET
                  && p2p_params->situation != NOT_SET);
    bool isCR = (p2p_params->confidence.size() > 0
                 && p2p_params->reliability.size() > 0);
...
{
    fprintf_s(fp, "%-24s %-12.2g %s\n", TAG__HTX, p2p_params->h_tx__meter,
              UNITS__METER);
    fprintf_s(fp, "%-24s %-12.2g %s\n", TAG__HRX, p2p_params->h_rx__meter,
              UNITS__METER);
