...
int main(int argc, char** argv)
{
    ...
    {
        rtn = CallAreaMode(&params, &area_params, &inter_vals, &A__db,
                           &A_fs__db, &warnings);
    }
        ...
        else
        {
                ...
                if (params.mode == MODE_P2P)
                {
                    ...
                    for (int i = 0; i < p2p_params.reliability.size(); i++)
                    {
                        ...
                        for (int j = 0; j < p2p_params.confidence.size(); j++)
                        {
                            fprintf_s(
                                fp, "%-10.1f",
                                A__db[i * p2p_params.confidence.size() + j]);
                        }
                        fprintf_s(fp, "\n");
                    }
                }
                else
                {
                    ...
                    for (int i = 0; i < area_params.d__km.size(); i++)
                    {
                        ...
                        for (int j = 0; j < area_params.confidence.size(); j++)
                        {
                            fprintf_s(
                                fp, "%-10.1f",
                                A__db[i * area_params.confidence.size() + j]);
                        }
                        fprintf_s(fp, "\n");
                    }
                }
            ...
            if (params.DBG
                && (params.mode == MODE_P2P
                    || (params.mode == MODE_AREA && A__db.size() == 1)))
            {
                fprintf_s(fp, "\n");
                ...
                fprintf_s(fp, "Mode of Propagation      %-12i ",
                          inter_vals.mode);
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (Match("-mode", argv[i]))
        {
            ...
            if (Match("p2p", argv[i + 1]))
                ...
                return ParseErrorMsgHelper("-mode [mode]",
                                           DRVRERR__PARSE_MODE_VALUE);
...
{
    for (int i = 0; i < strlen(argv); i++) argv[i] = tolower(argv[i]);
}
...
void Help()
{
    ...
    printf_s("Examples:\n");
    printf_s(
        "\tITMDrvr.exe -i inputs.txt -t terrain.txt -o results.txt -mode "
        "P2P\n");
    printf_s("\n");
};
...
int ValidateInputs(DrvrParams* params)
{
    ...
    if (strlen(params->out_file) == 0)
        return Validate_RequiredErrMsgHelper("-o",
    ...
    if (params->mode == MODE_P2P)
    {
        if (strlen(params->terrain_file) == 0)
            return Validate_RequiredErrMsgHelper(
                "-t", DRVRERR__VALIDATION_TERRAIN_FILE);
    }
...
void GetDLLVersionInfo()
{
    ...
    if (verSize != NULL)
    {
        ...
        {
            if (VerQueryValue(verData, TEXT("\\"), (VOID FAR * FAR*)&lpBuffer,
                              &size))
            {
                if (size)
                {
...
void GetDrvrVersionInfo()
{
    ...
    if (verSize != NULL)
    {
        ...
        {
            if (VerQueryValue(verData, L"\\", (VOID FAR * FAR*)&lpBuffer,
                              &size))
            {
                if (size)
                {
